...
var esOptions = {
  ...
  fields:
    'a comma-separated list of fields to return as part of a hit (default: "*")',
...
var esClientOptions = {
  apiVersion:
    'the major version of the Elasticsearch nodes you will be connecting to (default: 2.3)',
  maxRetries:
    'how many times should the client try to connect to other nodes before returning a ConnectionFault error (default: 3)',
  requestTimeout:
    'milliseconds before an HTTP request will be aborted and retried. This can also be set per request (default: 30000)',
  deadTimeout:
    'milliseconds that a dead connection will wait before attempting to revive itself (default: 60000)',
  pingTimeout:
    'milliseconds that a ping request can take before timing out (default: 3000)',
  maxSockets:
    'maximum number of concurrent requests that can be made to any node (default: 10)',
  minSockets:
    'minimum number of sockets to keep connected to a node (default: 10)',
  selector:
    'select a connection from the ConnectionPool using roundRobin (default) or random'
};
...
program
  ...
  .option(
    '-m, --max <number>',
    'the maximum number of items to export. different than the scroll size',
    parseInt
  )
  .option(
    '--transformMeta <js>',
    'a javascript function that returns an object that is the transformed meta object'
  )
  .option(
    '--transformSource <js>',
    'a javascript function that returns an object that is the transformed source object'
  )
  .option(
    '--transformMetaInit <js>',
    'a javascript function that returns an init object that contains helpers for the transform function'
  )
  .option(
    '--transformSourceInit <js>',
    'a javascript function that returns an init object that contains helpers for the transform function'
  );
...
for (key in esClientOptions) {
  if (esClientOptions.hasOwnProperty(key)) {
    program.option(
      '--' + key + ' <' + key + '>',
      'ES CLIENT OPTION: ' + esClientOptions[key]
    );
  }
}
...
['transformMetaInit', 'transformSourceInit'].forEach(function(key) {
  if (program.hasOwnProperty(key)) {
    program[key] = vm.runInNewContext(
      '(function () {' + program[key] + ';return {};}());'
    );
  }
});
...
var processResults = function(error, response) {
  ...
  if (!bar) {
    var halfAPercent = Math.round(total * 0.005);
    if (halfAPercent < barUpdateSize) barUpdateSize = halfAPercent;
    bar = new ProgressBar(
      'processing :current of :total [:bar] :percent :elapseds',
      {
        width: 20,
        total: total
      }
  ...
  response.hits.hits.forEach(function(hit) {
    ...
    if (program.transformMeta) {
      meta = vm.runInNewContext(
        '(function () {' + program.transformMeta + ';return data;}());',
        {
    ...
    if (program.transformSource) {
      source = vm.runInNewContext(
        '(function () {' + program.transformSource + ';return data;}());',
        {
    ...
    processed++;
    if (processed % barUpdateSize == 0) bar.tick(barUpdateSize);
  });
  if (stdout) {
